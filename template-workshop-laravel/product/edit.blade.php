...
<div class="bg-white shadow-lg rounded-lg p-6 md:p-8 max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-700">Edit Produk: {{ $produk->nama ?? 'Produk Dummy' }}</h1>
    ...
    <form action="{{-- route('produk.update', $produk->id ?? 1) --}}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="nama_produk" id="nama_produk" value="{{ old('nama_produk', $produk->nama ?? 'Nama Produk Dummy') }}" required
        ...
        <div>
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4" required
                      ...
                      placeholder="Masukkan deskripsi produk">{{ old('deskripsi', $produk->deskripsi ?? 'Deskripsi produk dummy.') }}</textarea>
            @error('deskripsi')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="harga" id="harga" value="{{ old('harga', $produk->harga ?? 100000) }}" required
            ...
            <div>
                ...
                <input type="number" name="stok" id="stok" value="{{ old('stok', $produk->stok ?? 10) }}" required
